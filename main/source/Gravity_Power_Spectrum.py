...
from numpy                import linspace, loadtxt, zeros, pi, sin, cos
from matplotlib.pyplot    import *
from matplotlib           import rc

rc('font',family='serif')

def Power_Spectrum_1D_Cal(t,x):
    n = int(len(t)/10)
    if (n<2):
       ...
       k = linspace(0.0,float(n-1)*pi/t[len(t)-1],n)
       pk= zeros(n)
...
def Power_Spectrum_Plot_Construction(Gravity_obj,file_loc_save="Report/",file_loc_load="Output/Power_Spectrum_data/"):
    ...
    for i in range(Gravity_obj.output.Power_Spectrum_points):
       ...
       (k,pk) = Power_Spectrum_1D_Cal(t_local,Ricci_local)
